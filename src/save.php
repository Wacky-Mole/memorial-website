...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Optional external video link (accept any host for submission; admin must approve embedding)
    ...
    if ($videoUrl !== '') {
        ...
        // Store the original URL; embedding will only occur after admin approves the entry
        $videoEntry = ['type' => 'video', 'path' => $videoUrl, 'caption' => filter_var($videoCaption, FILTER_SANITIZE_FULL_SPECIAL_CHARS)];
    }
    ...
    // If we have a video entry, append it into the photo JSON so rendering can show the link alongside photos
    if ($videoEntry !== null) {
        $entries = $entries ?? [];
        $entries[] = $videoEntry;
        $photoPath = json_encode($entries);
